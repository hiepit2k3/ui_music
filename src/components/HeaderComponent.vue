<template>
  <div class="top-0 left-0 w-full flex justify-between items-center p-4 z-50 bg-transparent">
    ...
    <!-- Nút Burger -->
    <div class="md:hidden">
      <button @click="toggleMenu" class="text-white hover:underline">
        ☰
      </button>
    </div>

    <!-- Menu Ẩn/Hiện Trên Màn Hình Nhỏ -->
    <div :class="{
      'opacity-100 md:hidden scale-100': isMenuOpen,
      'opacity-0 scale-95 md:hidden': !isMenuOpen,
    }"
      class="absolute top-16 left-0 w-full bg-white text-black rounded-md p-4 space-y-4 z-40 transition-all duration-300 ease-in-out transform">
      <ul class="w-full flex flex-col gap-2">
        <li class="flex-center cursor-pointer p-16-semibold w-full whitespace-nowrap">
          <button
            class="p-16-semibold flex size-full gap-4 p-4 group font-semibold rounded-lg bg-cover hover:bg-purple-100 hover:shadow-inner focus:bg-gradient-to-r from-purple-400 to-purple-600 focus:text-white text-gray-700 transition-all ease-linear">
